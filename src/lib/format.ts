...
// Content dates are written as M/D/YYYY, or as YYYY when only the year is known.
function parseDate(value: string): ParsedDate {
  ...
  throw new Error(`Unsupported date "${value}": use M/D/YYYY or YYYY`);
}

export function formatDate(value: string): string {
  const { year, month, day } = parseDate(value);
  return month && day ? `${MONTHS[month - 1]} ${day}, ${year}` : String(year);
}
